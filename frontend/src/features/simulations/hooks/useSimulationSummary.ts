...
export const useSimulationSummary = (
  ...
  const autoGeneratedSimulationIdRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    autoGeneratedSimulationIdRef.current = null;
  }, [simulationId]);
  ...
  useEffect(() => {
    if (
      !autoGenerate ||
      !simulationId ||
      autoGeneratedSimulationIdRef.current === simulationId ||
      loading ||
      requested
    ) {
      return;
  ...
  }, [autoGenerate, loading, requested, simulationId]);

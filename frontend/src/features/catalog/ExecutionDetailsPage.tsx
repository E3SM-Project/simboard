...
export const ExecutionDetailsPage = ({
  ...
  const readableIdentity =
    machineName && hpcUsername && caseName && routeExecutionId
      ? { machineName, hpcUsername, caseName, executionId: routeExecutionId }
      : null;
  const { data: fetchedExecution, loading, error } = useReadableExecution(readableIdentity);
  const executionUuid = fetchedExecution?.id;
  const [isHistoryOpen, setIsHistoryOpen] = useState(false);
  const history = useMetadataHistory('execution', executionUuid ?? '', isHistoryOpen);
  ...
  const summary = useExecutionSummary(executionUuid ?? '', {
  ...
  }, [error, fetchedExecution, loading]);
  ...
  const handleSave = async (payload: ExecutionUpdate): Promise<boolean> => {
    if (!executionUuid) return false;

    setIsSaving(true);
    setSaveError(null);

    try {
      const updatedExecution = await updateExecution(executionUuid, payload);
  ...
  if (!readableIdentity) {
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center text-gray-500">Invalid execution identity</div>
      </div>
    );
  }

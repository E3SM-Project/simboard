...
against a bind-mounted performance archive. Each run performs four phases:

1. Discover parseable execution directories grouped by case path.
2. Compare discovered execution IDs with persisted runner state.
...
def ingest_case_with_retries(
    ...
    *,
    ...
    post_request_fn: Callable[..., dict[str, Any]] = post_ingestion_request,
) -> dict[str, Any]:
    ...
    post_request_fn : Callable[..., dict[str, Any]], optional
        Request function, injectable for testing.
...
def run_ingestor(
    ...
    *,
    ...
    post_request_fn : Callable[..., dict[str, Any]], optional
        Request function used to call the ingestion endpoint.
...
def _log_event(event: str, **fields: Any) -> None:
    """Emit a structured JSON log record for one ingestion event.
...
def _build_config_from_args(args: argparse.Namespace) -> IngestorConfig:
    """Build and validate runtime config from parsed CLI arguments.
    ...
    """
    max_cases_per_run = _parse_optional_int(args.max_cases_per_run)
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    argparse.ArgumentParser
        Configured parser with environment-backed defaults.
    """
    parser = argparse.ArgumentParser(
        description="Scan /performance_archive and ingest changed cases into SimBoard."
    )

    parser.add_argument(
        "--api-base-url",
        default=os.getenv("SIMBOARD_API_BASE_URL", DEFAULT_API_BASE_URL),
        help="SimBoard base URL (without /ingestions/from-path).",
    )
    parser.add_argument(
        "--api-token",
        default=os.getenv("SIMBOARD_API_TOKEN", ""),
        help="Service-account bearer token for SimBoard API.",
    )
    parser.add_argument(
        "--archive-root",
        default=os.getenv("PERF_ARCHIVE_ROOT", DEFAULT_ARCHIVE_ROOT),
        help="Root directory for bind-mounted performance archive.",
    )
    parser.add_argument(
        "--machine-name",
        default=os.getenv("MACHINE_NAME", DEFAULT_MACHINE_NAME),
        help="Machine name sent to SimBoard ingestion endpoint.",
    )
    parser.add_argument(
        "--state-path",
        default=os.getenv("STATE_PATH", DEFAULT_STATE_PATH),
        help="Path to state JSON file used for idempotency.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        default=_parse_bool(os.getenv("DRY_RUN"), default=False),
        help="Discover candidates without calling SimBoard API.",
    )
    parser.add_argument(
        "--max-cases-per-run",
        default=os.getenv("MAX_CASES_PER_RUN"),
        help="Optional cap on number of candidate cases per run.",
    )
    parser.add_argument(
        "--max-attempts",
        default=os.getenv("MAX_ATTEMPTS", str(DEFAULT_MAX_ATTEMPTS)),
        help="Max retry attempts for transient API failures.",
    )
    parser.add_argument(
        "--request-timeout-seconds",
        default=os.getenv("REQUEST_TIMEOUT_SECONDS", str(DEFAULT_TIMEOUT_SECONDS)),
        help="HTTP timeout for one ingestion request.",
    )

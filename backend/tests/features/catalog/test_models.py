...
def simulation_create_all_db() -> Generator[Session, None, None]:
    schema_name = f"test_execution_create_all_{uuid4().hex}"
...
def _create_dependencies(session: Session) -> tuple[User, Machine, Ingestion]:
    user = User(email="execution-model@example.com", is_active=True, is_verified=True)
    machine = Machine(
        name="execution-model-machine",
    ...
    ingestion = Ingestion(
        ...
        source_reference="execution-model-test",
...
class TestExternalLinkOwnership:
    def test_execution_owned_external_link_is_valid(
        self, db: Session, normal_user_sync
    ) -> None:
        case = _create_case(db, "execution-owned-case")
        machine = _create_machine(db)
        ingestion = _create_ingestion(
            db,
            machine_id=machine.id,
            user_id=normal_user_sync["id"],
            source_reference="execution-owned-external-link",
        )
        execution = _create_execution(
            db,
            case_id=case.id,
            ingestion_id=ingestion.id,
            user_id=normal_user_sync["id"],
            execution_id="execution-owned-exec",
        )

        link = ExternalLink(
            execution_id=execution.id,
            kind=ExternalLinkKind.DIAGNOSTIC,
            url="https://example.com/execution-owned",
        ...
        assert case.executions == [execution]
        assert ingestion.executions == [execution]
        assert execution.links[0].id == link.id
        assert link.execution is execution

...
| Diagnostics case | One published diagnostics case directory below a diagnostics archive. Production is `production/<case>` or `production/<case_group>/<case>`; development is `development/<username>/<case>` or `development/<case_group>/<username>/<case>`. The development username namespaces published paths only. This filesystem identity is distinct from, and is resolved to, a SimBoard Case using the provenance case name, machine, and HPC username. |
...
| Scanner state | The successful scanner submission record for a machine and full archive-relative diagnostics case path. It records the selected settings filename, provenance timestamp, fingerprint, linked URL, submission time, and linked diagnostic link. The full path distinguishes development output published by different users for the same case. |
...
Invalid, unreadable, unsafe, or malformed provenance and settings inputs are skipped during discovery rather than becoming scanner candidates. The scanner only considers the `production` and `development` archive tiers, accepts the tier-specific layouts above, and selects the newest timestamped provenance configuration in each diagnostics case directory. The development username path component is not compared with provenance `hpc_username`: zppy derives them from different sources, and the provenance value remains the authoritative Case identity.
